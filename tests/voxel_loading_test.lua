-- The loading cover should reuse Gen1Recomp's Town Map without owning input,
...
local view
local Game = { data = {}, save = {}, overworld = { map = { id = "PALLET_TOWN" } } }
...
assert(clearColor[1] == 0 and clearColor[2] == 0 and clearColor[3] == 0
       and clearColor[4] == 1, "loading letterbox is not opaque black")

local second = Loading.draw(1920, 1080, 3)
assert(second == first, "loading canvas was needlessly rebuilt")
assert(townMapBuilds == 1, "Town Map resources were rebuilt per frame")
assert(townMapDraws == 2)

Loading.invalidate()
assert(released == 5, "loading/Town Map resources were not all released")
