...
-- The suite exercises the mod's platform-neutral contract. Gen1Recomp's
-- current headless stub reports a desktop OS for engine tests; hide that
-- synthetic answer here so VR keeps the harmless headless behavior the
-- upstream suite documents and tests below.
...
T.eq(run.loader.exports.DRAMATIC_SHAPE.version, "1.7.0-android.1",
  "the runtime export matches the Android package version")
ANDROID_FORK = true
...
;(function()
  ...
  T.eq(type(Data.screens) == "table" and Data.screens.HordeGameOver ~= nil,
    not ANDROID_FORK,
    ANDROID_FORK and "the Android fork does not register the Horde GAME OVER card"
      or "the GAME OVER card is in the screens registry")
  T.eq(type(Data.screens) == "table" and Data.screens.HordeExitPrompt ~= nil,
    not ANDROID_FORK,
    ANDROID_FORK and "the Android fork does not register the Horde exit prompt"
      or "the exit prompt START opens is in the screens registry")
  local sfx = Data.audio and Data.audio.sfx or {}
  local HordeSfx = lib.require("HordeSfx")
  for _, name in ipairs(HordeSfx.SHOTS) do
    T.eq(type(sfx[name]) == "table" and sfx[name].chip ~= nil,
      not ANDROID_FORK,
      ANDROID_FORK and name .. " stays unregistered in the Android fork"
        or "the gunshot " .. name .. " assembled into a chip program")
  end
  for _, name in ipairs({ HordeSfx.MAG_OUT, HordeSfx.MAG_IN, HordeSfx.RACK,
    ...
    T.eq(type(sfx[name]) == "table" and sfx[name].chip ~= nil,
      not ANDROID_FORK,
      ANDROID_FORK and name .. " stays unregistered in the Android fork"
        or name .. " assembled into a chip program")
  end
